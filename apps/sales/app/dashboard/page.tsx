...
function QuickActions() {
  const actions = [
    { label: 'Unternehmen', icon: ICONS.list, color: '#818CF8', href: '/dashboard/unternehmen' },
    { label: 'Lead generieren', icon: ICONS.zap, color: '#A78BFA', href: '/dashboard/generate' },
    { label: 'Meeting planen', icon: ICONS.calendar, color: '#38BDF8', href: '/dashboard/meetings' },
    { label: 'Network', icon: ICONS.globe, color: '#34D399', href: null },
    { label: 'Analytics', icon: ICONS.chart, color: '#FBBF24', href: '/dashboard/analytics' },
  ];
  return (
...
export default function SalesV2HomePage() {
  ...
  return (
    <>
      <Breadcrumbs items={[{ label: 'Onvero Sales', href: '/dashboard' }, { label: 'Home' }]} />
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {topLeads.map((lead, i) => (
              <Link
                key={lead.id}
                href={`/dashboard/unternehmen/${lead.id}`}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 14,
