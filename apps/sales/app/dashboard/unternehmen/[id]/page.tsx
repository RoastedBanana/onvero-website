...
function DetailInner({ id }: { id: string }) {
  ...
  function setTab(tab: TabKey) {
    router.replace(`/dashboard/unternehmen/${id}?tab=${tab}`);
  }
  ...
  if (error) {
    return (
      <>
        <Breadcrumbs
          segments={[
            { label: 'Onvero Sales', href: '/dashboard' },
            { label: 'Unternehmen', href: '/dashboard/unternehmen' },
            { label: 'Fehler' },
          ]}
        />
        <ErrorCard message={error} onBack={() => router.push('/dashboard/unternehmen')} />
      </>
    );
  }
  ...
  if (!company) {
    return (
      <>
        ...
        />
        <ErrorCard message="Unternehmen nicht gefunden" onBack={() => router.push('/dashboard/unternehmen')} />
      </>
    );
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        {navIds.length > 0 && currentIdx !== -1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: TOKENS.font.family }}>
            <button
              onClick={() => prevId && router.replace(`/dashboard/unternehmen/${prevId}`)}
              disabled={!prevId}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
            ...
            <button
              onClick={() => nextId && router.replace(`/dashboard/unternehmen/${nextId}`)}
              disabled={!nextId}
              style={{
                display: 'inline-flex',
                alignItems: 'center',
