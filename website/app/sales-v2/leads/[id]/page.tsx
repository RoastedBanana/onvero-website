...
function CollapsibleSection({
  ...
  return (
    ...
    >
      ...
      {open && <div style={{ padding: '20px 22px', animation: 'fadeInUp 0.25s ease both' }}>{children}</div>}
    </div>
  );
}

// ─── CAREER SECTION — collapsible with glow + hover ──────────────────────────

function CareerSection({ entries }: { entries: Lead['employmentHistory'] }) {
  ...
  return (
    <CollapsibleSection title="Berufsverlauf" icon={ICONS.trending} color="#A78BFA" count={sorted.length}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, position: 'relative' }}>
        {/* Vertical connector — glows at the top */}
        <div
          style={{
            position: 'absolute',
            left: 11,
            ...
            width: 2,
            borderRadius: 1,
            background: `linear-gradient(180deg, ${C.accent}40, ${C.border}, transparent)`,
          }}
        />
        {sorted.map((entry, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', gap: 14, padding: '2px 0' }}>
              {/* Dot — first one glows strongly */}
              <div style={{ width: 24, display: 'flex', justifyContent: 'center', flexShrink: 0, paddingTop: 14 }}>
                <div
                  style={{
                    width: isFirst ? 12 : 8,
                    height: isFirst ? 12 : 8,
                    ...
                    boxShadow: isFirst
                      ? `0 0 12px ${C.accent}80, 0 0 24px ${C.accent}30`
                      : entry.current
                        ? `0 0 6px ${C.accent}40`
                        : 'none',
                    zIndex: 1,
              ...
              <div
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = isFirst ? 'rgba(99,102,241,0.08)' : 'rgba(99,102,241,0.03)';
                  e.currentTarget.style.borderColor = isFirst ? 'rgba(99,102,241,0.25)' : 'rgba(99,102,241,0.1)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = isFirst ? 'rgba(99,102,241,0.05)' : 'rgba(255,255,255,0.01)';
                  e.currentTarget.style.borderColor = isFirst ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.04)';
                }}
              >
                {/* Duration bar */}
                <div
                ...
                <div style={{ position: 'relative' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span style={{ fontSize: 13, fontWeight: isFirst ? 600 : 500, color: isFirst ? C.text1 : C.text2 }}>
                      {entry.title}
                    </span>
                    {entry.current && (
                      <span
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    {durationStr && (
                      <span style={{ fontSize: 10, fontWeight: 600, color: isFirst ? C.accent : C.text3 }}>
                        {durationStr}
                      </span>
                    )}
                  </div>
...
// ─── COLLAPSIBLE TIMELINE — live from Supabase ───────────────────────────────

function CollapsibleTimeline({ leadId }: { leadId: string }) {
  const { activities, loading } = useActivities(leadId);
  return (
    <CollapsibleSection title="Aktivitäts-Timeline" icon={ICONS.clock} color="#38BDF8" count={activities.length}>
...
// Old LeadTimeline kept as alias for compatibility
