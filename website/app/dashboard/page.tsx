...
function Sidebar({ active, onNav, user, onLogout }: { active: Page; onNav:(p:Page)=>void; user: UserInfo|null; onLogout:()=>void }) {
  const [search, setSearch] = useState('');
  return (
    <aside style={{ width:220, flexShrink:0, height:'100vh', position:'sticky', top:0, background:'#0d0d0d', borderRight:'1px solid rgba(255,255,255,0.06)', display:'flex', flexDirection:'column', padding:'0', overflow:'hidden' }}>
      {/* Logo */}
...
export default function DashboardPage() {
  const router = useRouter();
  const [user] = useState<UserInfo | null>(parseCookieUser);
  ...
  const renderContent = () => {
    switch (activePage) {
      ...
      case 'leads':      return <EmptyState title="Leads" description="Verwalte Interessenten und verfolge deinen Sales-Funnel." />;
