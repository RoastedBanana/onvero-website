...
async function loadAllSessions(userId: string): Promise<ChatSession[]> {
  ...
  return rows
    ...
    .map((r) => {
      ...
      const title = firstUser ? firstUser.text.slice(0, 40) + (firstUser.text.length > 40 ? '…' : '') : 'Gespräch';
  ...
  return (
    ...
    >
      <div
        ...
        style={{ padding: '1rem 0.75rem', borderBottom: '1px solid rgba(255,255,255,0.06)', minWidth: 240 }}
      >
        <button
          onClick={onNew}
        ...
        <button
          onClick={onToggle}
          className="p-2 ml-1 rounded-lg transition-colors cursor-pointer"
      ...
      <div
        ...
        style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,0.08) transparent', minWidth: 240 }}
      >
        {groups.map((group) => (
          <div key={group.label} className="mb-2">
            ...
            {group.items.map((session) => (
              <div
                ...
                onMouseLeave={(e) => {
                  if (session.id !== activeId) (e.currentTarget as HTMLElement).style.background = 'transparent';
                }}
              >
        ...
        {sessions.length === 0 && (
          <p className="px-4 py-6 text-xs text-center" style={{ color: 'rgba(255,255,255,0.2)' }}>
...
export function BusinessAIChat() {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      updateSession(activeId, (s) => ({
        ...s,
        messages: [...s.messages, { role: 'ai', text: 'Es gab einen Fehler. Bitte versuche es erneut.' }],
      }));
    } finally {
      setLoading(false);
    }
