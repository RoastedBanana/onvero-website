...
import { Plus, AlertTriangle, Clock, Users, Mail, Calendar as CalendarIcon, ChevronDown, CheckCircle2, Circle, Video, Phone, MessageSquare, X, Loader2, Copy, Send, FileDown, Search, Sparkles } from 'lucide-react';
...
const S = {
  ...
  label: {
    fontSize: 9, textTransform: 'uppercase' as const, letterSpacing: '0.09em',
    color: 'rgba(255,255,255,0.35)', fontWeight: 600, marginBottom: 6,
  } as React.CSSProperties,
  val: {
    fontSize: 26, fontWeight: 700, fontFamily: 'var(--font-dm-mono)',
...
function dbToMeetingEntry(row: Record<string, unknown>): MeetingEntry {
  ...
  const date = isToday ? 'Heute' : isTomorrow ? 'Morgen' : scheduledAt.toLocaleDateString('de-DE', { weekday: 'short', day: 'numeric', month: 'short' });
  ...
  const allParticipants: Participant[] = [
    ...
    initials: p.initials ?? (p.name ?? '').split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0, 2),
  }));
...
function Avatar({ initials, type, size = 28 }: { initials: string; type: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      fontSize: size * 0.36, fontWeight: 600, color, fontFamily: 'var(--font-dm-mono)',
      flexShrink: 0,
    }}>
      {initials}
    </div>
...
function MeetingTypeBadge({ type }: { type: string }) {
  const labels: Record<string, string> = {
    standup: 'Standup', sales: 'Sales', board: 'Board', workshop: 'Workshop',
    one_on_one: '1:1', standard: 'Standard',
  };
  return (
    <span style={{
      fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em',
      color: 'rgba(255,255,255,0.5)', background: 'rgba(255,255,255,0.06)',
      border: '1px solid rgba(255,255,255,0.08)', borderRadius: 4, padding: '2px 7px',
    }}>
      {labels[type] ?? type}
    </span>
  );
  ...
  return (
    <div style={{ position: 'relative', width: 80, height: 80 }}>
      ...
      <div style={{
        ...
        fontSize: 18, fontWeight: 700, fontFamily: 'var(--font-dm-mono)', color: '#fff',
      }}>
        {pct}%
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', gap: 16, position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 12, flexShrink: 0 }}>
        <div style={{
          width: 10, height: 10, borderRadius: '50%', background: '#6B7AFF',
          border: '2px solid #111', zIndex: 1, marginTop: 16,
        }} />
      ...
      <div
        ...
        style={{
          ...S.card, flex: 1, marginBottom: 8, cursor: 'pointer',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{
              fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-dm-mono)', color: '#fff',
            }}>
              {meeting.time}
            </span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ display: 'flex' }}>
              {meeting.participants.slice(0, 4).map((p, i) => (
                <Avatar key={p.name} initials={p.initials} type={p.type}
                  size={24} />
              ))}
              {meeting.participants.length > 4 && (
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 9, color: 'rgba(255,255,255,0.4)', marginLeft: -6,
                }}>
                  +{meeting.participants.length - 4}
                </div>
              )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {meeting.objectives.map((obj, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{
                      fontSize: 12, color: obj.done ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.7)',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {meeting.participants.map((p) => (
                  <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{
                      fontSize: 9, color: p.type === 'lead' ? '#FF5C2E' : p.type === 'kunde' ? '#F59E0B' : 'rgba(255,255,255,0.3)',
...
const MEETING_TYPES_OPTIONS = [
  'Standup', 'Sales Call', '1:1', 'Board Meeting', 'Workshop', 'Brainstorming', 'Kundengespräch', 'Review',
];
...
function NewMeetingDialog({ onClose, onCreated }: { onClose: () => void; onCreated?: () => void }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .single();
      if (error) {
        console.error('[NewMeeting] Draft creation failed:', JSON.stringify(error), error.message, error.code, error.details, error.hint);
      } else if (data) {
  ...
  const addNewParticipant = () => {
    ...
    setSelectedParticipants([...selectedParticipants, p]);
    setNewPName(''); setNewPWebsite(''); setNewPLinkedin('');
    setNewPCompany(''); setNewPDescription('');
  ...
  const handleDownloadPdf = async () => {
    ...
    let y = 25;
    const addText = (text: string, size: number, style: 'normal' | 'bold' = 'normal', color = [30, 30, 30] as [number, number, number]) => {
    ...
    const plain = briefing.replace(/#{1,4}\s?/g, '').replace(/\*\*/g, '').replace(/\*/g, '');
      ...
      >
        ...
        <div style={{ padding: '20px 24px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div>
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="z.B. Sprint Review Q2" style={inputStyle} />
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={addObjective} style={{
                ...
                color: 'rgba(255,255,255,0.7)', borderRadius: 8, padding: '0 14px',
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                {objectives.map((o) => (
                  <span key={o} style={{
                    ...
                    borderRadius: 6, padding: '4px 10px', fontSize: 12, color: 'rgba(255,255,255,0.8)',
                  }}>
                    {o}
                    <button onClick={() => setObjectives(objectives.filter((x) => x !== o))} style={{
          ...
          {/* Beschreibung */}
          <div>
            <label style={fieldLabel}>Beschreibung <span style={{ color: 'rgba(255,255,255,0.2)' }}>optional</span></label>
            <textarea
          ...
          <div>
            ...
            >
              <option value="" style={{ background: '#1a1a1a' }}>Auswählen…</option>
              {MEETING_TYPES_OPTIONS.map((t) => (
                <option key={t} value={t.toLowerCase()} style={{ background: '#1a1a1a' }}>{t}</option>
              ))}
              <option value="custom" style={{ background: '#1a1a1a' }}>Eigene…</option>
            </select>
            {meetingType === 'custom' && (
              <input
                value={customType}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="meeting-date" className="px-1 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/35">
                Datum
              </Label>
              <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" id="meeting-date" className="w-full justify-between font-normal bg-white/[0.04] border-white/[0.08] text-white hover:bg-white/[0.06] hover:text-white">
                    {selectedDate
                      ? new Date(selectedDate.year, selectedDate.month - 1, selectedDate.day).toLocaleDateString('de-DE', { day: 'numeric', month: 'long', year: 'numeric' })
                      : 'Datum wählen…'}
                    <ChevronDown size={14} className="opacity-40" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto overflow-hidden p-0 bg-[#1a1a1a] border-white/10 z-[1100]" align="start">
                  <Calendar
            ...
            <div className="flex gap-3">
              <div className="flex flex-col gap-1.5 flex-1">
                <Label htmlFor="time-from" className="px-1 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/35">
                  Von
                </Label>
                <Input
                  type="time"
              ...
              <div className="flex flex-col gap-1.5 flex-1">
                <Label htmlFor="time-to" className="px-1 text-[9px] font-semibold uppercase tracking-[0.08em] text-white/35">
                  Bis
                </Label>
                <Input
                  type="time"
          ...
          <div ref={participantRef} style={{ position: 'relative' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                {selectedParticipants.map((p) => (
                  <span key={p.name} style={{
                    display: 'inline-flex', alignItems: 'center', gap: 5,
                    background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)',
                    borderRadius: 6, padding: '3px 8px', fontSize: 12, color: 'rgba(255,255,255,0.8)',
                  }}>
                    <Avatar initials={p.initials} type={p.type} size={18} />
                    {p.name}
                    <button onClick={() => toggleParticipant(p)} style={{
                      background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)',
                      cursor: 'pointer', fontSize: 14, padding: 0, lineHeight: 1,
                    }}>×</button>
                  </span>
            ...
            {showParticipantDropdown && (
              <div style={{
                ...
                background: '#1e1e1e', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10,
                  ...
                  return (
                    <div key={group}>
                      ...
                      {items.map((p) => (
                        <button
                          key={p.name}
                          onClick={() => { toggleParticipant(p); setParticipantSearch(''); }}
                          style={{
                ...
                {/* Sticky footer */}
                <div style={{
                  position: 'sticky', bottom: 0, padding: '4px',
          ...
          {showNewParticipantForm && (
            ...
            >
              <div
                ...
                style={{
                  background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.12)',
                  borderRadius: 14, width: '100%', maxWidth: 420, padding: 0,
              ...
              >
                <div style={{
                  ...
                  <button onClick={() => setShowNewParticipantForm(false)}
                ...
                <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    ...
                    <input value={newPName} onChange={(e) => setNewPName(e.target.value)} placeholder="Vor- und Nachname"
                      style={inputStyle} autoFocus />
                  </div>
                  <div>
                    <label style={fieldLabel}>Art</label>
                    <select value={newPType} onChange={(e) => setNewPType(e.target.value as 'mitarbeiter' | 'kunde' | 'lead')}
                      style={{ ...inputStyle, appearance: 'none', cursor: 'pointer' }}>
                      <option value="mitarbeiter" style={{ background: '#1a1a1a' }}>Mitarbeiter</option>
                      <option value="kunde" style={{ background: '#1a1a1a' }}>Kunde</option>
                      <option value="lead" style={{ background: '#1a1a1a' }}>Lead</option>
                    </select>
                  </div>
                  <div>
                    <label style={fieldLabel}>Unternehmen</label>
                    <input value={newPCompany} onChange={(e) => setNewPCompany(e.target.value)} placeholder="Firmenname"
                      style={inputStyle} />
                  </div>
                  <div>
                    <label style={fieldLabel}>Website <span style={{ color: 'rgba(255,255,255,0.2)' }}>optional, für Analyse</span></label>
                    <input value={newPWebsite} onChange={(e) => setNewPWebsite(e.target.value)} placeholder="https://..."
                      style={inputStyle} />
                  </div>
                  <div>
                    <label style={fieldLabel}>LinkedIn <span style={{ color: 'rgba(255,255,255,0.2)' }}>optional, für Analyse</span></label>
                    <input value={newPLinkedin} onChange={(e) => setNewPLinkedin(e.target.value)} placeholder="https://linkedin.com/in/..."
                      style={inputStyle} />
                  </div>
                  <div>
                    <label style={fieldLabel}>Kurze Beschreibung</label>
                    <input value={newPDescription} onChange={(e) => setNewPDescription(e.target.value)} placeholder="z.B. CEO, Ansprechpartner für Projekt X"
                      style={inputStyle} />
                  </div>
                </div>
                <div style={{ padding: '12px 20px 16px', display: 'flex', gap: 8 }}>
                  <button onClick={addNewParticipant} disabled={!newPName.trim()}
                    style={{
                      flex: 1, padding: '10px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 600, cursor: newPName.trim() ? 'pointer' : 'default',
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16 }}>
            <button
              ...
              style={{
                ...
                color: '#F59E0B', fontSize: 13, fontWeight: 600, cursor: emailLoading ? 'wait' : 'pointer',
              }}
            >
              {emailLoading
              ...
              <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: '#fff' }}>
                        {emailResult.could_be_email ? 'Ja, das könnte eine E-Mail sein' : 'Nein, ein Meeting ist sinnvoll'}
                      </span>
                      <span style={{
                        fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em',
                        padding: '2px 6px', borderRadius: 4,
                        color: emailResult.confidence === 'hoch' ? '#F59E0B' : 'rgba(255,255,255,0.5)',
                        background: emailResult.confidence === 'hoch' ? 'rgba(245,158,11,0.1)' : 'rgba(255,255,255,0.06)',
                      }}>
                ...
                {emailResult.email_draft && (
                  <div style={{
                    ...
                    {/* Body */}
                    <div style={{ padding: 14, fontSize: 12, color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                    <button
                      ...
                      style={{
                        ...
                        padding: '8px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)',
                        borderRadius: 8, color: 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 500, cursor: 'pointer',
                      }}
                    >
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16 }}>
            <button
              ...
              style={{
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                width: '100%', padding: '10px', borderRadius: 8,
                background: 'rgba(107,122,255,0.08)', border: '1px solid rgba(107,122,255,0.2)',
                color: '#6B7AFF', fontSize: 13, fontWeight: 600, cursor: briefingLoading ? 'wait' : 'pointer',
              }}
            >
              {briefingLoading
                ? <Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} />
            ...
            {briefing && (
              <div style={{ marginTop: 12 }}>
                <div className="meeting-md" style={{
                  background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)',
                  borderRadius: 8, padding: 14, fontSize: 12, color: 'rgba(255,255,255,0.7)', lineHeight: 1.6,
        ...
        <div style={{ padding: '14px 24px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          <button
            ...
            style={{
              ...
              color: '#0a0a0a', border: 'none', borderRadius: 8, fontSize: 13,
...
export function MeetingsPrepare() {
  ...
  const totalHours = meetings
    .filter((m) => m.date === 'Heute')
    .reduce((sum, m) => sum + parseInt(m.duration) / 60, 0);
  ...
  if (emailMeetings.length > 0) {
    emailMeetings.forEach((m) => warnings.push({ icon: Mail, text: `"${m.title}" könnte eine E-Mail sein`, severity: 'info' }));
  }
  const noObjectives = meetings.filter((m) => m.objectives.length === 0);
  if (noObjectives.length > 0) {
    noObjectives.forEach((m) => warnings.push({ icon: Clock, text: `${m.title} hat keine Agenda — Objectives hinzufügen`, severity: 'warn' }));
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
        <button
          ...
          style={{
            ...
            background: 'rgba(255,255,255,0.92)', color: '#0a0a0a', border: 'none',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ ...S.card, display: 'flex', flexDirection: 'column' }}>
          {loadingMeetings ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <span style={{
                  fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-dm-mono)', color: '#6B7AFF',
                }}>
                  {nextMeeting.time}
                </span>
              </div>
        ...
        <div style={{ ...S.card, display: 'flex', gap: 0 }}>
          ...
          <div style={{ flex: 1, paddingLeft: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ paddingLeft: 4 }}>
        {loadingMeetings ? (
          <div style={{ display: 'flex', alignItems: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 12, padding: 16 }}>
